...
AWS를 시작하면서 제일 처음 해야 할 일은 당연히 AWS 계정을 만드는 일입니다. 그 다음 해야 하는 일은 IAM에서 사용자를 생성하는 것입니다. 엥? AWS 계정 말고 IAM 사용자를 또 만든다고? 이 글을 끝까지 읽어 보시면 그 이유와 용도를 알 수 있습니다.
...
이 글에서는 루트 사용자 수준의 권한을 가진 IAM 사용자를 만들 것입니다. 그리고, 그 IAM 사용자를 이용해서 개발자용으로 다른 IAM 사용자를 추가하도록 해 보겠습니다.
...
![login_as_a_root_user](/assets/img/iam_admin_user/01_login.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![login_as_a_root_user](/assets/img/iam_admin_user/02_my_account.png) 
{: refdef}

계정 설정 페이지에서 아래로 스크롤을 하면 결제 정보에 대한 IAM 사용자 및 역할 액세스 항목이 나옵니다. 오른쪽의 "편집"을 누릅니다.

{:refdef: style="text-align: center;"}
![edit](/assets/img/iam_admin_user/03_edit.png) 
{: refdef}

"IAM 액세스 활성화"를 누르고 "업데이트"를 누릅니다.

{:refdef: style="text-align: center;"}
![activate](/assets/img/iam_admin_user/04_activate.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![to_iam](/assets/img/iam_admin_user/05_to_iam.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![06_add_user](/assets/img/iam_admin_user/06_add_user.png) 
{: refdef}

[사용자 이름]을 입력합니다. 관리자 역할을 할 사용자를 추가하는 것이므로, 저는 헷갈리지 않도록 Administrator를 입력했습니다. AWS Management Console 액세스 옆의 확인란을 선택하고 사용자 지정 암호를 선택한 다음 텍스트 상자에 새 암호를 입력합니다. 필요시, 사용자가 다음에 로그인할 때 비밀번호를 변경하도록 설정할 수 있습니다.

"다음:권한"을 누릅니다. 

{:refdef: style="text-align: center;"}
![07_add_user2](/assets/img/iam_admin_user/07_add_user2.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![08_create_group](/assets/img/iam_admin_user/08_create_group.png) 
{: refdef}

그룹 생성 대화 상자의 그룹 이름에 "Administrators"를 입력합니다. 유저를 만들 때는 단수형을 썼지만, 그룹 이름은 끝에 "-s"를 붙여서 복수형을 썼다는 것, 알아 채셨나요?

AdministratorAccess 정책을 추가합니다. 정책이란, 어떤 리소스에 어떤 액세스를 가능하게 할 지 권한을 미리 정해 놓은 것입니다. 일단은, AdministratorAccess 정책은 모든 리소스에 모든 액세스가 가능하다고 알고 넘어갑시다. 

{:refdef: style="text-align: center;"}
![09_create_group2](/assets/img/iam_admin_user/09_create_group2.png) 
{: refdef}

"그룹 생성"을 누르면 "Administrators" 그룹이 생성됩니다. 그룹 목록이 있는 페이지로 돌아가는데, 목록에 새 그룹이 표시되지 않으면 "새로 고침"을 하면 됩니다. "Administrators" 그룹을 선택하고 "다음: 태그"를 누릅니다.

{:refdef: style="text-align: center;"}
![10_add_group](/assets/img/iam_admin_user/10_add_group.png) 
{: refdef}

태그 추가 화면이 나옵니다. 태그는 사용자 또는 AWS 리소스에 뭍이는 이름이라고 생각할 수 있습니다. 선택 사항이므로, 일단은 "다음: 검토"를 눌러서 넘어갑니다. 
...
{:refdef: style="text-align: center;"}
![11_create_user](/assets/img/iam_admin_user/11_create_user.png) 
{: refdef}

아래와 같은 화면이 나오고, 사용자 추가 작업이 완료됩니다.

{:refdef: style="text-align: center;"}
![12_user_created](/assets/img/iam_admin_user/12_user_created.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![14_email](/assets/img/iam_admin_user/14_email.png) 
{: refdef}

로그인 URL로 접속하면 아래와 같은 화면이 나옵니다. 계정 ID 12자리는 이미 입력이 되어 있을 것입니다. 사용자 이름(Administrator)과 암호를 입력하여 로그인 합니다.

{:refdef: style="text-align: center;"}
![13_login_as_administrator](/assets/img/iam_admin_user/13_login_as_administrator.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![15_logged_in_as_administrator](/assets/img/iam_admin_user/15_logged_in_as_administrator.png) 
{: refdef}

# 관리자 계정으로 개발용 IAM 사용자 생성
...
{:refdef: style="text-align: center;"}
![16_cli_access](/assets/img/iam_admin_user/16_cli_access.png) 
{: refdef}

## 파워 유저 그룹 생성

위에서는 모든 권한이 부여된 Administrators 그룹을 생성했었습니다. 이번에는 유저 관리는 할 수 없지만 다른 모든 서비스에는 액세스가 가능한 PowerUsers란 그룹을 만들겠습니다. 이 그룹에는 주로 개발자들이 포함될 것 같네요.

"그룹 생성"을 누릅니다. 그룹 이름은 PowerUsers로 했습니다. 그룹 이름이므로 복수형을 사용했습니다. 

이미 만들어진 정책은 800개가 넘는데요. 그 중에서 PowerUserAccess라는 정책을 추가할 것입니다. 찾기에서 "poweruseraccess"를 입력한 다음, PowerUserAccess 옆에 체크를 하고 "그룹 생성"을 누릅니다.

{:refdef: style="text-align: center;"}
![17_power_user_access](/assets/img/iam_admin_user/17_power_user_access.png) 
{: refdef}
...
{:refdef: style="text-align: center;"}
![18_poweruser_created](/assets/img/iam_admin_user/18_poweruser_created.png) 
{: refdef}

[중요] 방금 만든 사용자는 "프로그래밍 방식 액세스" 유형으로 만들었습니다. 프로그래밍 방식으로 액세스 하려면 "액세스 키 ID"와 "비밀 액세스 키"가 필요합니다. 그런데, "비밀 액세스 키"는 이 화면에서만 다운로드 받을 수 있습니다. 따라서, 여기에서 .csv 파일을 다운로드 받아 두어야 합니다. 만약 이 단계에서 "비밀 액세스 키"를 못받은 경우에는 액세스 키를 다시 만들어야 합니다. IAM > 사용자 > 보안 자격 증명 으로 가서 액세스 키를 다시 만들 수 있습니다. 
